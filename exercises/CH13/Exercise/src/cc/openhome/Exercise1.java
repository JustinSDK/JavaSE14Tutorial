...
public class Exercise1 {
    ...
    private static void showTitle(Calendar calendar) {
        ...
        for(var weekDay : weekDays) {
            out.printf("%3s", weekDay);
        }
        out.println();
    }
    
    private static void showDate(Calendar calendar) {
        var cal = (Calendar) calendar.clone();
        // 今天是本月第幾天
        var dayOfMonth = cal.get(DAY_OF_MONTH);
        // 將月曆設為本月第一天
        cal.add(DAY_OF_MONTH, -dayOfMonth + 1);
        // 日曆現在是星期幾，注意get()傳回值是對應Calendar的列舉值
        // dayOfWeek 是用來排版用的
        var dayOfWeek = cal.get(DAY_OF_WEEK);
        // 顯示星期描述
        out.printf("%" + (2 * dayOfWeek + 1) + "s", "");
        for(var i = 1; i < cal.getMaximum(DAY_OF_MONTH); 
                i++, dayOfWeek++) {
            out.printf("%3d", i);
            if((dayOfWeek % 7) == 0) {
                out.println();
